...
#from ansible.errors import AnsibleError, AssertionError
...
def _read_password_file(b_path):
    ...
    if os.path.exists(b_path):
        with open(b_path, 'rb') as pwfile:
            b_content = pwfile.read().rstrip()
...
def _check_complexity(characters, password):
    ...
    for chars_spec in characters:
        ...
        if not foundCharSpec:
                #print( "not found: {}".format(charsset))
            pass
        else:
            #print("found: {}".format(charsset))
            pass
...
def _format_content(password, salt, encrypt=True):
    ...
    # At this point, the calling code should have assured us that there is a
    # salt value.
    if not salt:
